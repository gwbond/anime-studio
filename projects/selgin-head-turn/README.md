...
When rigging a head turn, one must create two smart-bones actions: one for clockwise head turn and the other for counter-clockwise head turn. The only difference between the two is that the keyframe sequence for one action is the reverse of the sequence for the other action. Currently, one must manually create both sequences. A proposed new Anime Studio feature would simply create the reversed sequence for a smart-bone given a sequence for an existing smart-bone.

While Anime Studio doesn't currently support this, there is a script from Vernon Zehr's (aka "heyvern") HubuMedia, called [Flip Symmetrical Keys](http://www.hubumedia.com/anime-studio-scripts), that creates a reverse keyframe sequence on a given layer. Using this script it may achieve the aforementioned functionality with some limitations: the script works for keyframes on a limited number of animation channels e.g., while it supports point motion and curvature, it doesn't support stroke exposure.

### Symmetric Smart-Bone Actions and Symmetric Drawing ###

When creating a smart-bone action for a head turn, one creates a keyframe sequence that defines the 360 degree progression of the face turning from front view to profile view to back of head view to opposite profile and then returning to front. For anything but the simplest heads, defining the keys for half the head turn i.e., from front to profile to back, can result in many keyframes on many layers. When the head is symmetric or nearly symmetric, which is often the case, creating the keyframes for the remaining half of the head turn requires creating mirror-image keyframes for points on the opposite side of the head. This task is complicated by the fact that there is currently no support for comparing one profile view with the mirror image view when adjusting point positions. Assuming a symmetric face, the proposed new feature would generate the keyframes for half of the turn based on the keyframes already defined for the other half of the turn. In order to make this work, there would initially have to be perfect point symmetry for the face, which brings us to the next feature...

A related new feature would support symmetric drawing along an axis. Any points placed on one side of the axis would simulatenously appear in mirror-image on the other side. A closed curve on one side of the axis would result in the creation of a mirror-image closed curve on the other side. A closed curve extending across the axis would be created when the two end points of an open curve on one side of the axis are placed on the axis. Using this feature a completely symmetric face could be constructed. Then, using the aforementioned symmetric smart-bone actions feature, half of the the head turn keyframing could be automatically generated based on the first half. After this is done, one could disable symmetric drawing support, thereby allowing one to introduce asymmetries to the head and to the head turn keyframing.

While Anime Studio doesn't currently support symmetric drawing, there is a script from Vernon Zehr's (aka "heyvern") HubuMedia, called [Mirror Points Horizontally](http://www.hubumedia.com/anime-studio-scripts), that attempts to align points drawn on either side of a vector layer's y axis so that they are symmetrical. The script does not support interactive symmetrical drawing, however. HubuMedia also has a script called [Flip Symmetrical Keys](http://www.hubumedia.com/anime-studio-scripts) that, given a symmetric drawing, will swap the position and curvature of selected symmetric points, thereby providing a way to manually create keyframes for the second half of a head turn based on those created for the first half.

While HubuMedia's scripts attempt to address the problems described above, the benefit of building these two new features into Anime Studio would provide a more comprehensive and easy-to-use solution, speeding up the animation process and allowing one to concentrate on the creative aspects of drawing and animation.